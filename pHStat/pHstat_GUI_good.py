...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QGridLayout, 
                             ...
                             QPushButton, QTabWidget, QFrame, QMenu, QMessageBox, QActionGroup)
from PyQt5.QtGui import QFont, QColor, QIcon, QPen, QTransform, QPalette
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QMetaObject, pyqtSlot, QTimer, QMutex, QSize
...
from pHStat_classes import (pHPickerDialog, SelectPickerDialog, pumpControl, 
                            DatePickerDialog, CustomTextWidget, ClickableLabel, CalibratePumpDialog,CalibratepHDialog, monoTimer)
...
class MainWindow(QMainWindow):
    ...
    def setupWidgets(self):
        ...
        pHLayout.addWidget(self.pHlabel,0,0, alignment= Qt.AlignRight)
        pHLayout.addWidget(self.pHNumber,0,1,1,2, alignment=Qt.AlignLeft)
        pHLayout.addWidget(self.RTDlabel,1,1,alignment=Qt.AlignTop)
    ...
    def initializeGraphTabs(self):
        ...
        self.addDualGraphTab("pH + Temp Plot")

        # (Optional) Add RTD plot separately if you still want individual
        # self.addGraphTab("Temperature Plot", ("Time (s)", "Temperature (°C)"))
    ...
    #def initializeGraphTabs(self):
     #   axisLabels = [("Time (s)", "Added (ml)"), ("Time (s)", "pH"), ("Time (s)", "Temperature (°C)")]
      #  for i, labels in enumerate(axisLabels):
       #     self.addGraphTab(f'{self.plotindex[i]} plot', labels)
            # Initial plot for each graph
            #x, y = [0],[0]
        #    self.updatePlot(i)

#     def updatePlot(self, index):
#         self.graphWidgets[index].clear()
#         plotColors = ['r', 'g', 'b']  # Example colors: red, green, yellow
# 
#         time , data = read_log_data(self, index)
#         x , y, time_label, scale = scale_time_data(self, time, data)
#         labelStyle = {'color': 'black', 'font-size': '11pt'}
#         self.graphWidgets[index].setLabel('bottom', f'Time ({time_label})', **labelStyle)  # X-axis label, color, and font size
#         
#         self.graphWidgets[index].plot(x,y, pen=pg.mkPen(plotColors[index], width=2))
    ...
    def updatePlot(self, tab):
        ...
        self.graphWidgets[plot_index].clear()
        
        plotColors = ['r', 'g', 'b']
        labelStyle = {'color': 'black', 'font-size': '11pt'}
        if plot_index == 0:
            # Pump plot (standard single plot)
            time, data = read_log_data(self, 0)
            x, y, time_label, scale = scale_time_data(self, time, data)
            self.graphWidgets[plot_index].setLabel('bottom', f'Time ({time_label})', **labelStyle)
            self.graphWidgets[plot_index].plot(x, y, pen=pg.mkPen(plotColors[0], width=2))

        elif plot_index == 1:
            # Dual pH + Temp plot
            self.tempViewBox.clear()  # Clear right side temp plot separately

            # pH (left Y-axis)
            ph_time, ph_data = read_log_data(self, 1)  # index 1 = pH
            x_ph, y_ph, time_label, scale = scale_time_data(self, ph_time, ph_data)
            self.graphWidgets[plot_index].setLabel('bottom', f'Time ({time_label})', **labelStyle)

            self.graphWidgets[plot_index].plot(x_ph, y_ph, pen=pg.mkPen('g', width=2))  # green for pH

            # Temperature (right Y-axis)
            temp_time, temp_data = read_log_data(self, 2)  # index 2 = RTD
            x_temp, y_temp, _, _ = scale_time_data(self, temp_time, temp_data)
            temp_curve = pg.PlotCurveItem(x_temp, y_temp, pen=pg.mkPen('r', width=2))  # red for Temp
            self.tempViewBox.addItem(temp_curve)

            # Auto-range the right Y-axis
            self.tempViewBox.enableAutoRange(axis=pg.ViewBox.YAxis)
   ...
   #def updateCurrentTabPlot(self):
    ...
    def update_gui(self, received_data, type):
        ...
        if type == 1:
            self.pHNumber.setText(f'{str("{:.2f}".format(received_data))}')
